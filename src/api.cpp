...
char *post_req(const char *path, const char *data, bool return_response)
{
        ...
        char api_URL[256];
        snprintf(api_URL, 256, "%s%s", ha_url.c_str(), path);
        ...
        char *auth_data = new char[strlen(bearer) + strlen(long_lived_access_token.c_str()) + 1];
        if (!auth_data) {
                ESP_LOGE(TAG, "auth_data malloc failed.");
                return NULL;
        }
        memcpy(auth_data, bearer, strlen(bearer));
        memcpy(auth_data + strlen(bearer), long_lived_access_token.c_str(),
               strlen(long_lived_access_token.c_str()) + 1);
...
std::string get_req(const char *path)
{
        ...
        char api_URL[128];
        snprintf(api_URL, 128, "%s%s", ha_url.c_str(), path);

        // Long-Lived Access Token for Home Assistant API. This is generated by an administrator and is valid for 10
        // years.
        constexpr const char *bearer = "Bearer ";
        // char *auth_data = (char *)malloc(strlen(bearer) + strlen(long_lived_access_token) + 1);
        char *auth_data = new char[strlen(bearer) + 1 + strlen(long_lived_access_token.c_str()) + 1];
        if (!auth_data) {
                ESP_LOGE(TAG, "auth_data malloc failed.");
                return NULL;
        }
