...
char* post_req(const char* path, const char* data, bool return_response) {
    ...
    if (post_err == ESP_OK) {
        if (data)
            ESP_LOGI(TAG, "POST %s to %s", data, api_URL);
    } else {
    ...
    if (return_response && post_err == ESP_OK) {
        ...
        if (post_response_err == ESP_OK) {
            post_response_err = esp_http_client_fetch_headers(client);
            if (post_response_err != ESP_FAIL) {
                int64_t content_length = esp_http_client_get_content_length(client);
                response_buffer = malloc(content_length+1);

                if (response_buffer) {
                    esp_http_client_read_response(client, response_buffer, content_length);
                    // Ensure response is null-terminated at the least. This does NOT mean the response is a valid JSON.
                    // If cJSON tries to parse an invalid JSON it will crash!
                    response_buffer[content_length] = '\0'; // ensure response is null-terminated
                    ESP_LOGV(TAG, "POST response content length %lld Read: %s", content_length, response_buffer);
                    failed_response = false;
                } else {
                    ESP_LOGE(TAG, "POST response_buffer malloc failed.");
                }
            }
        }
...
char* get_req(const char* path)
{
    ...
    if (err == ESP_OK) {
        ...
        if (err != ESP_FAIL) {
            ...
            get_failed = false; // GET response can still fail if response_buffer malloc fails, but it will just return NULL.
            if (response_buffer) {
                esp_http_client_read_response(client, response_buffer, content_length);
                // Ensure response is null-terminated at the least. This does NOT mean the response is a valid JSON.
                // If cJSON tries to parse an invalid JSON it will crash!
                response_buffer[content_length] = '\0'; 
                ESP_LOGI(TAG, "Read %s, \nSize: %lld", response_buffer, content_length);
            } else {
                ESP_LOGE(TAG, "HTTP GET request failed: response_buffer malloc failed");
            }
        }
    }
...
bool get_api_status(void)
{
    ...
    ESP_LOGI(TAG, "Status: %s", cJSON_GetStringValue(message));
